...
import { OpenClawRuntimePluginWriter } from "../../apps/controller/src/runtime/openclaw-runtime-plugin-writer.js";
...
describe("OpenClawRuntimePluginWriter", () => {
  ...
  it("skips symlinked node_modules/.bin entries while copying plugin directories", async () => {
  ...
  it("preserves real node_modules/.bin directories", async () => {
    ...
    await writeFile(path.join(binDir, "tool"), "#!/usr/bin/env node\n");
