...
export class RuntimeOrchestrator {
  ...
  private refreshLaunchdUnit(record: RuntimeUnitRecord): void {
    ...
    try {
      ...
      const uid = require("node:os").userInfo().uid;
    ...
    if (status.status === "running") {
      ...
      // unknown — keep current phase but clear pid
      record.pid = null;
    }
  ...
  private tailLaunchdLogs(record: RuntimeUnitRecord): void {
    ...
    for (const logFile of logFiles) {
      try {
        ...
        const fd = require("node:fs").openSync(logFile.path, "r");
        try {
          require("node:fs").readSync(fd, buffer, 0, buffer.length, readStart);
        } finally {
  ...
  private async stopLaunchdUnit(record: RuntimeUnitRecord): Promise<void> {
    ...
    try {
      await this.launchdManager.stopServiceGracefully(label);
      this.refreshLaunchdUnit(record);
